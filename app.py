...
if uploaded_files:
    st.session_state.recent_predictions = []  # Reset
    images = []
    ...
    # ✅ Export combined PDF with images (batch only)
    if len(batch_results) > 1:
        pdf = FPDF(orientation='L', unit='mm', format='A4')
        pdf.set_auto_page_break(auto=True, margin=15)
        pdf.set_font("Arial", size=12)

        for idx, (filename, label, confidence, pil_img) in enumerate(batch_results):
            pdf.add_page()
